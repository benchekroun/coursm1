...
void TrackBallCamera::OnKeyboard(const SDL_KeyboardEvent & event)
{
    ...
    else if((event.type == SDL_KEYDOWN)&&(event.keysym.sym == SDLK_PAGEUP))// Augmenter sensibilité
    {
        _scrollSensivity+=0.2;
    }
    else if((event.type == SDL_KEYDOWN)&&(event.keysym.sym == SDLK_PAGEDOWN))//Décrementer sensibilité
    {
        if(_scrollSensivity>1)
            _scrollSensivity-=0.2;
        else _scrollSensivity=1;
    }
}
